	...
	)
{
	...
	if ( (pktptr->net_ipdst == NetData.ipucast) ||
		...
		ip_local(pktptr);
		//kprintf("SENDING UDP PACKET UP\n");
		return;
	} else {

		/* Drop the packet */
		//kprintf("UNMACTECH DROPPING UDP");
		freebuf((char *)pktptr);
		return;
	}
}
