	...
	)
{
	...
	//kprintf("P:dport:%d::S:dport:%d::P:sport:%d:S:sport:%d::P:ip%d:S:ip:%d\n", pktptr->net_udpdport ,udptr->udlocport,
			//pktptr->net_udpsport,udptr->udremport, pktptr->net_ipsrc, udptr->udremip);
	...
	//kprintf("FINAL DROP IN UDP %d:%d\n",pktptr->net_ipsrc,pktptr->net_ipdst);
	freebuf((char *) pktptr);
	restore(mask);
	return;
}
